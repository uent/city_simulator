...
`State` SHALL expose `PublicSummary() string` and `LocalContext(locationID string, presentNames []string) string` methods. The `Summary() string` method SHALL NOT exist.
...
`LocalContext(locationID string, presentNames []string)` SHALL include all information it previously returned PLUS a "Characters present" section listing the names in `presentNames`. When `presentNames` is empty or nil, the section SHALL be omitted.
...
#### Scenario: LocalContext includes present characters
- **WHEN** `LocalContext("Market", []string{"Alice", "Bob"})` is called
- **THEN** the returned string SHALL contain "Alice" and "Bob" in a characters-present section

#### Scenario: LocalContext omits characters section when list is empty
- **WHEN** `LocalContext("Market", nil)` is called
- **THEN** the returned string SHALL NOT contain a characters-present section

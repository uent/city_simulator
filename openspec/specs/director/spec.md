...
- The `spawn_character` tool SHALL appear in the `<tools>` block ONLY when `state.Concept.CharacterSpawnRule` is non-empty; when included, the tool description SHALL show the rule text and a "use sparingly" warning; if `MaxSpawnedCharacters > 0`, the remaining capacity SHALL also be shown
...
#### Scenario: Remaining capacity shown when cap is set
- **WHEN** `state.Concept.MaxSpawnedCharacters` is `3` and `state.SpawnedCharacters` is `1`
- **THEN** the prompt SHALL contain capacity info indicating `2` remaining

...
The system SHALL define an `Action` interface in `internal/director/action.go` with two methods: `Name() string` and `Execute(args map[string]any, state *world.State, chars []*character.Character) error`. Every named director action SHALL implement this interface.

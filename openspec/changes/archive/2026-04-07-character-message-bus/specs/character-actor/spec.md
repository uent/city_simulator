...
1. Build a system prompt from the character's persona and the world context included in the message payload
...
When a `MoveDecision` message arrives, the `CharacterActor` SHALL call the LLM with the character's movement prompt (current location + available locations), parse the response, and write back a reply containing the chosen location name or `"stay"` on error.

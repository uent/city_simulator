...
The system SHALL provide a `Chat(messages []Message, opts ...Option) (string, Usage, error)` method that sends a POST to `<baseURL>/api/chat` with `stream: false` and returns the assistant's response text and zero `Usage`.
...
- **THEN** the method SHALL return the `message.content` field, zero `Usage`, and nil error

#### Scenario: Non-200 HTTP status
- **WHEN** Ollama returns HTTP 500
- **THEN** the method SHALL return an empty string, zero `Usage`, and a non-nil error containing the status code

#### Scenario: Network timeout
- **WHEN** the HTTP request exceeds the configured timeout (default 120 seconds)
- **THEN** the method SHALL return an empty string, zero `Usage`, and a non-nil error describing the timeout

---

### Requirement: Generate method on Client
The system SHALL provide a `Generate(systemPrompt, userPrompt string, opts ...Option) (string, Usage, error)` method on `*Client` that constructs a two-message slice (`system` + `user`) and delegates to `Chat`.

#### Scenario: Successful generation
- **WHEN** `Generate("You are a helpful assistant.", "What is 2+2?")` is called and Ollama responds with HTTP 200
- **THEN** the method SHALL return the assistant's response text, zero `Usage`, and nil error

#### Scenario: System prompt empty
- **WHEN** `Generate("", "hello")` is called
- **THEN** the method SHALL still issue the request with an empty system message and return whatever Ollama replies with zero `Usage`
...
- **THEN** the method SHALL return an empty string, zero `Usage`, and a non-nil error (propagated from `Chat`)
